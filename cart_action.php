<?php
    ...
    if(isset($_POST['selectedItemID'])){
        if(isset($_SESSION['email'])){
            ...
            $sql = "INSERT INTO cart (userID, jerseyID, quantity) 
    ...
    if(isset($_POST['getUserCartData'])){
        if(isset($_SESSION['id'])){
            $id = $_SESSION['id'];
            $sql = "SELECT jerseyID, name, quantity, price FROM cart JOIN jersey ON jerseyID = id WHERE userID = '$id'";
    ...
    if(isset($_POST['delete_item'])){
        ...
        $query = "DELETE FROM `cart` WHERE `userID`='$userID' AND `jerseyID`='$itemID'";
    ...
    if(isset($_POST['updateQuantity'])){
        ...
        $q = "UPDATE `cart` SET `quantity` = '$quantity' WHERE `userID`= '$userID' AND `jerseyID` = '$jerseyID'";
        if(mysqli_query($db, $q)){
            echo "jerseyID: ".$jerseyID." quantity: ".$quantity." Successfully updated.";
        }else{
